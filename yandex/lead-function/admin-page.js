...
<body>
  ...
  (function(){
    ...
    async function request(action,options,params){var config=Object.assign({},options||{}),headers=Object.assign({'X-Admin-Request':'1'},config.headers||{}),token=sessionToken();if(token)headers['X-Admin-Session']=token;config.headers=headers;var response=await fetch(endpoint(action,params),config);var body={};try{body=await response.json()}catch(e){}if(response.status===401){if(action!=='login'){saveSession('');showLogin()}throw new Error(action==='login'?'INVALID_LOGIN':'AUTH')}if(!response.ok)throw new Error(body.error||'REQUEST');if(body.session)saveSession(body.session);return body}
