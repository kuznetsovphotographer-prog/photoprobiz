...
- При окончательном удалении заявки удаляйте связанные `lead_meta`, `consent_events` и `leads` одной DML-транзакцией внутри базы-владельца; коллизии ID между базами должны отклоняться до записи.
