...
| CRM-кабинет | Работает | Yandex Cloud Function, две YDB, поиск, периоды, ручные клиенты, заметки и удаление |
...
- Активная CRM-версия: `d4ee23dmjq2hhq1qhbbr`
...
CRM умеет показывать один сайт или все сайты, выбирать месяц внутри года, группировать выдачу по месяцам и дням, искать по имени или телефону, создавать клиента вручную и хранить источник, статус и заметку. На телефонах и планшетах карточка предлагает позвонить или выбрать мессенджер, на компьютере дополнительно показывает отдельную кнопку копирования. Для MAX номер копируется, затем открывается `https://max.ru/`: публичной прямой ссылки поиска пользователя по номеру у MAX нет. Кабинет читает `photoprobiz.ru` из основной базы и `prointeriors.ru` из отдельной интерьерной базы; ручные записи имеют служебный источник `manual.crm` и остаются в основной базе. Новые браузерные источники добавляются через `CRM_SITES_JSON` и совместимый POST формы.
...
Политика и согласие бизнес-сайта обновлены до `2026-09-12-2`; техническая сверка и оставшиеся риски описаны в `PERSONAL-DATA-REVIEW-2026-09-12.md`. Бизнес-форма собирает только укрупнённые устройство/ОС для диагностики формы. Основная `leads` расширена nullable-полями `device_type` и `os_family`; старые строки сохраняются и показываются как «Не определено». Сроки документов согласованы с кодом: год для заявки и заметок, три года как предельный срок доказательства от серверного получения, с обязанностью оценивать законное основание и досрочное удаление.
...
12.09.2026 активирована версия CRM `d4ee23dmjq2hhq1qhbbr`. В ней добавлены фильтры по году/месяцу, группировка списка, поиск по имени и телефону, адаптивное меню мессенджеров, расширенный технический блок и окончательное индивидуальное/массовое удаление связанных данных. Проверены размеры 390×844, 820×1180, 1024×1366 и 1440×900 без горизонтального переполнения и резкой границы фона. Две синтетические заявки удалены через production API; контрольные запросы к YDB вернули ноль строк во всех трёх связанных таблицах.
