...
- Сайт опубликован по адресу `https://kuznetsovphotographer-prog.github.io/photoprobiz/`.
- Форма отправляет запросы на `https://photoprobiz-lead-form.kuznetsovphotographer.workers.dev/lead`.
- Telegram-бот и Cloudflare Secrets уже подключены; отправка через Worker проверена.
- У временного адреса нет собственного домена и DNS-зоны в Cloudflare. В репозитории нет файла `CNAME`, а GitHub Pages работает на поддомене `github.io`.

На этой схеме нельзя создать адрес вида `api.ваш-домен.ru`: для Custom Domain Cloudflare требует активную DNS-зону, которой владеет владелец Worker. Поддомен `github.io` не принадлежит проекту.

Из-за этого нельзя честно подтвердить требование работы формы без VPN для российских клиентов, пока она вызывает `workers.dev`. Доступность можно проверять только с реальных подключений российских провайдеров без VPN; на текущем компьютере такую проверку нельзя подменять обычным успешным HTTP-запросом.
...
После появления собственного домена потребуется поменять только этот адрес и добавить Custom Domain Worker. Логика формы и дизайн сайта не изменятся.
