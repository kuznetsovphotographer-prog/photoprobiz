...
Сейчас форма работает в демонстрационном режиме и не отправляет заявку фотографу. Для реального приёма заявок понадобится подключить HTTPS endpoint через `VITE_LEAD_ENDPOINT` и заново собрать сайт. GitHub Pages выполняет только статическую часть и не запускает сервер обработки формы.
